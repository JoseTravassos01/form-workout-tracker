...
export class TrainingService {
  ...
  private async prepareWorkout(profileId: string, workout: WorkoutDto, currentWeek: number): Promise<WorkoutDto> {
    const [policyRow, recovery] = await Promise.all([
      this.database.prepare(`SELECT p.progression_policy progressionPolicy,a.sex profileSex FROM training_programs p
        JOIN athlete_profiles a ON a.current_program_id=p.id WHERE a.id=? LIMIT 1`).bind(profileId).first<{ progressionPolicy: string; profileSex: "male" | "female" }>(),
    ...
    if (workout.blockNumber === 1 && currentWeek <= 2 && policyRow?.profileSex === "male") {
    ...
    } else if (workout.blockNumber === 1 && currentWeek <= 2 && policyRow?.profileSex === "female") {
  ...
  async today(profileId: string, now = new Date()): Promise<{ workout: WorkoutDto | null; cardio: Record<string, unknown> | null; state: Record<string, unknown> } | null> {
    ...
    return { workout, cardio, state: { week: state.currentWeek, block: state.currentBlock, blockName: await this.blockName(state.program_id, state.currentBlock), today: state.today } };
  }
  ...
  async workout(profileId: string, sessionId: string): Promise<WorkoutDto | null> {
    const [workout, state] = await Promise.all([
      new WorkoutRepository(this.database).getWorkout(profileId, sessionId),
      this.effectiveState(profileId),
    ]);
    return workout && state ? this.prepareWorkout(profileId, workout, state.currentWeek) : null;
  }
  ...
  async dashboard(profileId: string, now = new Date()) {
    ...
    const summary = await new DashboardRepository(this.database).summary(profileId, format(startOfWeek(parsed, { weekStartsOn: 1 }), "yyyy-MM-dd"), format(endOfWeek(parsed, { weekStartsOn: 1 }), "yyyy-MM-dd"), todayString);
    const [latestRecovery, upcoming] = await Promise.all([
      new RecoveryRepository(this.database).latest(profileId),
    ...
    return { ...today, weeklyCompleted: summary.workouts.filter((item) => item.status === "completed").length, weeklyScheduled: 4, completedTotal: summary.completedTotal, streak: summary.streak, nextSession, weights: summary.measurements, recovery: latestRecovery };
  }

  async strength(profileId: string, exerciseId?: string): Promise<StrengthProgressDto | null> {
    const exerciseResult = await this.database.prepare(`SELECT e.id,MIN(COALESCE(NULLIF(ep.display_name,''),e.name)) name FROM exercises e JOIN exercise_prescriptions ep ON ep.exercise_id=e.id JOIN training_days d ON d.id=ep.training_day_id JOIN training_blocks b ON b.id=d.block_id JOIN athlete_profiles a ON a.current_program_id=b.program_id WHERE a.id=? GROUP BY e.id ORDER BY name`).bind(profileId).all<{ id: string; name: string }>();
