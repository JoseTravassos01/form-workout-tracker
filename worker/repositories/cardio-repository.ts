export class CardioRepository {
  ...
  async start(profileId: string, prescriptionId: string, scheduledDate: string, expectedVersion: number | null) {
    ...
    if (!prescription) return { found: false, conflict: false, id: "", version: 0 };
    ...
    await this.database.prepare("INSERT INTO cardio_sessions (id,athlete_profile_id,cardio_prescription_id,scheduled_date,started_at,status,modality) VALUES (?,?,?,?,CURRENT_TIMESTAMP,'in_progress',?)").bind(id, profileId, prescriptionId, scheduledDate, prescription.modality).run();
